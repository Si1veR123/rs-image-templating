...
impl<T: PixelChannel> TextSettings<T> {
    ...
    fn glyph_positions(&self) -> (HashMap<char, Vec<(isize, isize)>>, (isize, isize), (isize, isize)) {
        ...
        for (glyph, glyph_x, glyph_y) in LayoutIter::new(self).filter_map(|g| g.ok()) {
        ...
        (positions, minimum_coord, maximum_coord)
    }

    /// Create a rasterized image from the text settings
    pub fn raster_from_settings(&self) -> Image<T> {
...
impl<T: PixelChannel> TextLayer<T> {
    pub fn new(settings: TextSettings<T>, x: usize, y: usize) -> Self {
        let raster = settings.raster_from_settings();
        Self { settings: settings, rasterized: raster, x, y, filters: vec![] }
    }
    ...
    pub fn set_settings(&mut self, settings: TextSettings<T>) {
